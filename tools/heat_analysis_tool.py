...
integrated_heat_analysis.py for thermal analysis.
"""
...
# Import the thermal conductivity analysis module.
try:
    from tools.integrated_heat_analysis import (
        HeatConductivityAnalyzer,
        Config as HeatConfig,
        VoxelGenerator,
        NumericalHomogenization
    )
    HEAT_ANALYSIS_AVAILABLE = True
except ImportError:
    try:
        from integrated_heat_analysis import (
            HeatConductivityAnalyzer,
            Config as HeatConfig,
            VoxelGenerator,
            NumericalHomogenization
        ...
        print(f"Warning: unable to import the thermal analysis module: {e}")
        HEAT_ANALYSIS_AVAILABLE = False


def run_integrated_heat_analysis(
    input_path: str,
    output_dir: str = None,
    resolution: int = 128,
    mode: str = 'solid',
    base_heat: float = 1.0,
    material_conductivity: float = 1.0,
    use_gpu: bool = True,
    ...
    """
    Integrated thermal conductivity analysis tool.
    
    Args:
        input_path: Input file or directory.
        output_dir: Result directory. Defaults to ``heat_results`` in the
            current working directory.
        resolution: Voxel resolution. Defaults to 128.
        mode: Voxelization mode, either ``surface`` or ``solid``. Defaults to
            ``solid``.
        base_heat: Base thermal conductivity used by the solver. Defaults to 1.0.
        material_conductivity: Physical base-material thermal conductivity;
    ...
    if not HEAT_ANALYSIS_AVAILABLE:
        return {
            "success": False,
            "error": "The thermal analysis module is not installed correctly.",
            "results": []
        }
    ...
    try:
        ...
        if output_dir is None:
            output_dir = os.path.join(os.getcwd(), 'heat_results')
        
        # Update the analysis configuration.
        HeatConfig.OUTPUT_DIR = output_dir
        HeatConfig.VOXEL_RESOLUTION = resolution
        HeatConfig.VOXEL_MODE = mode
        HeatConfig.BASE_HEAT = base_heat
        HeatConfig.USE_GPU = use_gpu
        HeatConfig.ENABLE_VISUALIZATION = enable_visualization
        HeatConfig.GPU_DEVICE = gpu_device
        HeatConfig.SOLVER_TOL = solver_tol
        HeatConfig.SOLVER_MAX_ITER = solver_max_iter
        
        # Create the analyzer.
        analyzer = HeatConductivityAnalyzer(HeatConfig)
        ...
        return {
            ...
            "output_files": {
                "csv_summary": os.path.join(output_dir, 'heat_conductivity_summary.csv') if results else None,
        ...
        return {
            "success": False,
            "error": f"Thermal conductivity analysis failed: {str(e)}",
            "results": []
        }
...
def _perform_comparison_analysis(results: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    try:
        ...
        # Sort by thermal conductivity.
        sorted_results = sorted(results, key=lambda x: x.get('thermal_conductivity', 0), reverse=True)
        ...
        return {
            "best_structure": {
                ...
                'thermal_conductivity': best['thermal_conductivity'],
                'volume_fraction': best['volume_fraction']
            },
            "worst_structure": {
                'filename': worst['filename'],
                'thermal_conductivity': worst['thermal_conductivity'],
...
def _generate_statistics(results: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    try:
        ...
        conductivity_values = [r['thermal_conductivity'] for r in results if 'thermal_conductivity' in r]
...
run_heat_conductivity_analysis = run_integrated_heat_analysis


def get_heat_analysis_files(directory: str = None) -> List[str]:
    """
    Return OBJ files available for thermal conductivity analysis.
